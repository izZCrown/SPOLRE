...
from layout_editor_ablation import editor
import traceback
import copy
import stanza
...
if __name__ == "__main__":
    ...
    image_bank_path = '../images_ablation'
    mask_bank_path = '../mask_bank_ablation'
    gen_image_path = '../gen_image_ablation'
    obj_mask_path = '../obj_mask_ablation'

    mkdir(mask_bank_path)
    mkdir(gen_image_path)
    mkdir(obj_mask_path)

    caption_path = '../image_caption_final.json'
    # target_objs_path = '/home/wgy/multimodal/MuMo/target_objs.json'
    target_objs_path = './target_objs_ablation.jsonl'
        ...
        for key in tqdm(data_list.keys()):
            ...
            try:
                ...
                if len(target_objs) != 0:
                    ...
                    editor(image_dir=obj_image_path, output_path=mask_dir, step=10, gen_num=1)
                    editor(image_dir=obj_image_path, output_path=mask_dir, step=10, gen_num=2)
                    editor(image_dir=obj_image_path, output_path=mask_dir, step=10, gen_num=3)
                    editor(image_dir=obj_image_path, output_path=mask_dir, step=10, gen_num=4)
                    ...
                    for item in tqdm(mask_list):
                        ...
                        re_draw(image_path=mask_path, output_path=output_dir, num_samples=3)
            except Exception:
                print('--------error--------')
                traceback.print_exc()
                print('---------------------')
