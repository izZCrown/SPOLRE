...
obj_dir = "./input/obj_img/"
bg_dir = "./input/background/"
dst_dir = "./output/layout/"
json_idx_imgid_path = "./output/idx_imgid.json"

change_times = 3 # 一张图片对应的变换上限
layout_nums = 5 # 一张图片对应生成的新layout数量

now_idx = 0 # 当前已经生成几张新layout图
idx_imgid_json = dict()
...
# 对比变换前和便换后的图片
def watch(src_img, dst_img):
    plt.subplot(1, 2, 1)
    plt.xticks([]), plt.yticks([])
    plt.imshow(src_img), plt.title("src_img")
    plt.subplot(1, 2, 2)
    plt.xticks([]), plt.yticks([])
    plt.imshow(dst_img), plt.title("dst_img")
    plt.show()
...
def obj_mask_change(obj):
    ...
    src_info = obj["info"]
    dst_dict = {}
    dst_dict["cat"] = obj["cat"]
    print("change cat:",obj["cat"])
...
# 生成一张新的layout图
def single_layout_generate(imgid,bg,obj_list,obj_num,change_times):
    # 变换次数
    for time in range(0, change_times):
...
def main():
    # 遍历obj_dir下的文件夹 文件夹名为imgid
    all_img = os.listdir(obj_dir)
    for imgid in all_img:
        print(imgid)
        background = get_background(int(imgid))
        obj_list = get_obj_img(int(imgid))
        obj_num = len(obj_list)
        for num in range(0,layout_nums):
            single_layout_generate(imgid,background.copy(),copy.deepcopy(obj_list),obj_num,change_times)

    # 保存生成的layout图中idx与image_id的对应关系
    with open(json_idx_imgid_path, 'w') as f:
        f.write(json.dumps(idx_imgid_json))

if __name__ == '__main__':
    main()

    # imgid = 2592
    # img = cv2.imread(obj_dir+str(imgid)+"/cup_2.png")
    # bg = get_background(imgid)
    # obj_info = read_obj_info(img)
    # print(obj_info)
